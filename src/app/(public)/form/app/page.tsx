 ...
 * デザイン刷新版(v2)の申請フォーム。
 *
 * - state管理・バリデーション・住所検索・メール重複チェック・送信処理・
 *   サンクス画面・テスター用リダイレクト等のロジックは従来どおり。
 * - 送信先 API は /api/apply。
 * - スキーム判定(SCPP/MRT)・クエリパラメータ(?ref/?staff/?rep)対応。
 * - /m/form/app から本ファイルが再エクスポートされ、MRT スキームとして動作する。
 */
