  ...
  const handleFetchNews = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchNews(); // リスト更新
        setActiveTab("fetched"); // 取得済みタブに切り替え
      } else {
    ...
    <div>
      ...
      {previewItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setPreviewItem(null)}>
          <div className="bg-bg-secondary border border-border rounded-lg max-w-2xl w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            {/* プレビュー画像 */}
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[11px] text-text-muted">{previewItem.sourceName}</span>
              ...
              <div className="p-3 bg-bg-tertiary rounded border border-border mb-4">
                <div className="text-[10px] text-text-muted mb-1">元記事URL</div>
                <a
                  href={previewItem.sourceUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[12px] text-gold hover:text-gold-light break-all"
                >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`px-6 py-2.5 rounded-sm text-[13px] font-medium cursor-pointer transition-all ${
                ...
                <button
                  ...
                  className="px-6 py-2.5 border border-border text-text-muted rounded-sm text-[13px] cursor-pointer hover:border-border-gold hover:text-gold transition-all"
                >
                  閉じる
                </button>
              </div>
