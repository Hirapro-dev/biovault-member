...
export function GlossaryForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/admin/glossary", {
        ...
        body: JSON.stringify({ term: term.trim(), description: description.trim(), category }),
      });
      if (res.ok) {
        setTerm(""); setDescription(""); setCategory("basic"); setOpen(false);
        router.refresh();
      }
  ...
  return (
    <div className="mb-5">
      {!open ? (
        <button onClick={() => setOpen(true)} className="px-4 py-2.5 bg-gold-gradient border-none rounded-sm text-bg-primary text-sm font-semibold tracking-wider cursor-pointer hover:opacity-90 transition-all">
          + 新規登録
        </button>
      ) : (
        <div className="bg-bg-secondary border border-border-gold rounded-md p-5">
          ...
          <div className="flex gap-2">
            <button onClick={() => setOpen(false)} className="px-4 py-2 bg-transparent border border-border text-text-secondary rounded-sm text-xs cursor-pointer hover:border-border-gold transition-all">キャンセル</button>
...
export function GlossaryEditButton({ glossaryTerm }: { glossaryTerm: GlossaryTerm }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="px-3 py-1 bg-transparent border border-border text-text-secondary rounded-sm text-[11px] hover:border-border-gold hover:text-gold transition-all">編集</button>
      {open && (
...
// ── 上下移動ボタン ──
export function GlossaryMoveButtons({ id, currentOrder, isFirst, isLast }: { id: string; currentOrder: number; isFirst: boolean; isLast: boolean }) {
  const router = useRouter();
  ...
  const handleMove = async (direction: "up" | "down") => {
    ...
    try {
      await fetch(`/api/admin/glossary/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sortOrder: direction === "up" ? currentOrder - 1.5 : currentOrder + 1.5 }),
      });
      // 全用語の表示順を整数に再正規化
  ...
  return (
    <div className="flex gap-0.5">
      <button onClick={() => handleMove("up")} disabled={isFirst || moving} className="w-7 h-7 flex items-center justify-center bg-transparent border border-border text-text-muted rounded-sm text-xs hover:border-border-gold hover:text-gold transition-all disabled:opacity-20 cursor-pointer disabled:cursor-default">↑</button>
      <button onClick={() => handleMove("down")} disabled={isLast || moving} className="w-7 h-7 flex items-center justify-center bg-transparent border border-border text-text-muted rounded-sm text-xs hover:border-border-gold hover:text-gold transition-all disabled:opacity-20 cursor-pointer disabled:cursor-default">↓</button>
    </div>
  );
}
...
export function CategoryBadge({ category }: { category: string }) {
  ...
  return (<span className={`text-[10px] px-2 py-0.5 rounded-full border ${style}`}>{getCategoryLabel(category)}</span>);
}
