 ...
 * 代理店側 顧客カルテ（閲覧専用）
 *
 * 管理者ページと全く同じ UI。readOnly={true} で操作不可。
 * 管理者メモは代理店（社外）には表示しない。
 */
export default async function AgencyCustomerKartePage({
  ...
  if (!profile?.agencyCode) notFound();

  const user = await prisma.user.findUnique({
    where: { id },
    include: {
      membership: { include: { treatments: { orderBy: { createdAt: "desc" } } } },
  ...
  if (!user || user.role !== "MEMBER" || user.referredByAgency !== profile.agencyCode) notFound();

  const membership = user.membership;

  let staffName: string | null = null;
  if (user.referredByStaff) {
    const s = await prisma.staff.findUnique({ where: { staffCode: user.referredByStaff }, select: { name: true } });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5 sm:mb-7 flex-wrap gap-2">
        <Link href="/agency/customers" className="text-[11px] text-text-muted hover:text-gold transition-colors">← 紹介顧客一覧に戻る</Link>
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-bg-elevated text-text-muted border border-border">👁 閲覧モード</span>
      </div>

      <h2 className="font-serif-jp text-lg sm:text-[22px] font-normal text-text-primary tracking-[2px] mb-5 sm:mb-7">顧客カルテ</h2>
        ...
        <div className="bg-bg-secondary border border-border rounded-md p-4 sm:p-6">
          ...
          <div className="flex items-center py-2 border-t border-border mt-1">
            ...
            <div className="text-[13px]">
              {user.hasAgreedTerms ? <span className="text-status-active">同意済 {user.agreedTermsAt && <span className="text-text-muted text-[11px] ml-1">({new Date(user.agreedTermsAt).toLocaleDateString("ja-JP")})</span>}</span> : <span className="text-status-warning">未同意</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 書類管理（閲覧専用） */}
      <DocumentManager userId={user.id} documents={user.documents.map(doc => ({ id: doc.id, type: doc.type, title: doc.title, status: doc.status, fileUrl: doc.fileUrl, signedAt: doc.signedAt ? doc.signedAt.toISOString() : null }))} readOnly={true} />
      ...
      <div className="mt-6 bg-bg-secondary border border-border rounded-md p-4 sm:p-6">
        ...
        {membership?.treatments.length ? membership.treatments.map((t) => (
          <div key={t.id} className="flex items-center justify-between py-3 border-b border-border last:border-b-0">
            <div><div className="text-[13px]">{TREATMENT_TYPE_LABELS[t.type]}</div><div className="text-[11px] text-text-muted mt-0.5">{t.volume}cc・{t.clinicName || "---"}</div></div>
            <div className="text-[11px] text-text-muted font-mono">{t.completedAt ? new Date(t.completedAt).toLocaleDateString("ja-JP") : t.scheduledAt ? `予定: ${new Date(t.scheduledAt).toLocaleDateString("ja-JP")}` : "---"}</div>
          </div>
      ...
      <div className="mt-6 bg-bg-secondary border border-border rounded-md p-6">
        ...
        {user.statusHistory.length ? user.statusHistory.map((h) => (
...
function HealthRow({ label, has, detail }: { label: string; has: boolean; detail: string | null }) {
  return (
    <div className="flex items-start py-2 border-b border-border last:border-b-0">
      ...
      <div className="text-xs">{has ? <span className="text-status-warning">あり{detail && `（${detail}）`}</span> : <span className="text-text-secondary">なし</span>}</div>
    </div>
  );
}
